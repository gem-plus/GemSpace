...
router.post("/following", isLoggedIn, async (req, res) => {
  try {
    const following = await postService.following(
      req.user.userid,
      req.body.postOwner,
    );
    return res.json({ success: true, isfollow: following });
  } catch (error) {
    console.error("route error in /follow:", error);
    return res.status(500).json({ success: false });
  }
});
...
router.post("/like/:id", isLoggedIn, async (req, res) => {
  try {
    const post = await postService.like({
